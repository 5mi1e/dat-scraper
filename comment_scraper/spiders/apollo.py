...
class ApolloSpider(scrapy.Spider):
    ...
    def __init__(self, dfrom=None, dto=None, *args, **kwargs):
        ...
        self.start_urls = (
            'http://apollo.tvnet.lv/arhivs/' +  str(self.start_date.year) + '/' +
    ...
    def parse(self, response):
        for article in response.xpath('//div[@class="items"]/article[@class="article-medium"]//h2[@class="article-title"]'):
            comment_url = article.xpath('a[@class="article-comments-count"]/@href').extract()
            if comment_url:
                comment_count = int(re.search('([0-9])', str(article.xpath('a[@class="article-comments-count"]/span/text()').extract())).group())
                if comment_count > 0:
                    yield scrapy.Request(comment_url[0], callback=self.parse_comments)
        self.start_date += self.step
        if self.start_date <= self.end_date:
            url = 'http://apollo.tvnet.lv/arhivs/' +  str(self.start_date.year) + '/' + str(self.start_date.month) + '/' + str(self.start_date.day)
    ...
    def parse_comments(self, response):
        for comment in response.xpath('//div[@class="article-comments"]/div[ contains(@class, "article-comment")]'):
            ...
            item['comment_id'] = int(re.search('([0-9]+)', comment.xpath('@id').extract()[0]).group())
            auth = comment.xpath('div/span[@class="article-comment-pic"]//span[contains(@class, "icon-soc")]/@title').extract()
            if auth:
                item['author_auth'] = auth[0]
            item['comment_author'] = ''.join(comment.xpath('div/span[@class="article-comment-pic"]/img/@title').extract())
            ...
            if pos:
                item['recommend_pos'] = pos[0]
            neg = comment.xpath('div/div/div/div/a[ contains(@class, "minus") ]/text()').extract()
            if neg:
                item['recommend_neg'] = neg[0]

            item['comment_url'] = response.url
            comment_txt = comment.xpath('div[@class="article-comment-content"]/text() | div[@class="article-comment-content"]/a/@href').extract()
            comment_txt = ' '.join(comment_txt)
            item['comment_txt'] = re.sub('(/comment/external.url=)', "", comment_txt)
            yield item
