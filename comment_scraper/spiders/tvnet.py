...
class TvnetSpider(scrapy.Spider):
    ...
    def parse_comments(self, response, date):
        if response.xpath('//ol[@class="commentary"]/li/div[@class="comment-container"]/div/div[@class="comment-tool-container"]//@data-comment-id').extract():
            for comment in response.xpath('//ol[@class="commentary"]/li'):
                try:
                    ...
                    item['recommend_pos'] = int(comment.xpath('div[@class="comment-container"]/div/div[@class="comment-tool-container"]//div[@class="recommend-count"]/text()').extract()[0])
                    comment_txt = comment.xpath('div[@class="comment-container"]/p[@class="message"]/text() | div[@class="comment-container"]/p[@class="message"]/a/@href').extract()
                    if comment_txt:
                        comment_txt = map(unicode.strip, comment_txt)
                        comment_txt = ' '.join(comment_txt)
                        item['comment_txt'] = comment_txt
                    item['comment_url'] = comment.xpath('div[@class="comment-container"]/div/span[@class="date"]/a/@href').extract()
                    avatar = comment.xpath('div[@class="author-picture-container"]/img/@src').extract()
                    author_auth = comment.xpath('div[@class="comment-container"]/div/span[@class="author"]/img/@src').extract()
                    ...
                    if comment.xpath('div[@class="comment-container"]/div/span[@class="author"]/img[@class="top-commentator"]'):
                        item['top100'] = 1
                    yield item
                except:
                    pass
